      ...
      <div className="inputs-wrapper">
        <input
          ...
          value={sampleRange
            ...
            .join(" | ")}
        />
        <button className="clear-box" onClick={() => setSampleRange([null, null])}>clear</button>
      </div>
    </div>
  );
};
  ...
  return (
    <div className="datepicker" onKeyDown={handleKeyPress}>
      ...
      <div className="controls">
        <button className="calenbutn2" onClick={increment(-1)}>previous month</button>
        <button className="calenbutn2" onClick={() => setMonthView(new Date())}>today</button>
        <button className="calenbutn2" onClick={increment(1)}>next month</button>
      </div>
    </div>
  );
};
...
const Month = ({
  ...
  const totalDays = differenceInCalendarDays(sfw, elw) + 5;
  const days = times(identity, totalDays).map((_: any, idx: number) => addDays(idx, sfw));
...
const MonthSelector = ({
  ...
  const months = times(identity, 12).map((_: any, idx: number) => addMonths(idx, sy));
  return (
    <select
